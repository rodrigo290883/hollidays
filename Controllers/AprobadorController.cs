...
namespace desconectate.Controllers
{
    public class AprobadorController : Controller
    {
        ...
        public List<Solicitud> SolicitudesPendientes(int id_sap)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT s.folio, e.nombre, ts.solicitud, s.fecha_inicio,s.fecha_fin,es.descripcion FROM solicitudes s INNER JOIN empleados e ON s.idsap = e.idsap INNER JOIN ctipos_solicitud ts ON s.tipo_solicitud = ts.id_tipo_solicitud INNER JOIN cestatus es ON es.estatus = s.estatus  WHERE s.estatus = 0 and idsap_aprobador = @idsap ORDER BY fecha_solicitud DESC",conn);
